...
### Changed
- Synchronized models metadata with latest models.dev database: replaced **GLM-5.1** (`z-ai/glm-5.1`) with **GLM-5.2** (`z-ai/glm-5.2`).

## [0.0.3] - 2026-07-02

### Added
- Synchronized models metadata with latest models.dev database, adding **MiniMax-M3** (`minimaxai/minimax-m3`) to the supported chat models.

### Fixed
- Added missing `backup-models` npm script entry to `package.json` so `bun run backup-models` works out of the box.
